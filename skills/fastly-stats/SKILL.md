---
name: fastly-stats
description: "Use when retrieving Fastly traffic metrics, analytics, or usage data — cache hit ratio, bandwidth, request counts, error/status-code rates, edge vs origin traffic, real-time requests-per-second, origin latency, per-domain traffic, or billing/usage totals. Covers the Fastly CLI `fastly stats` commands AND the raw Historical Stats, Real-Time analytics, and Origin/Domain Inspector HTTP APIs on api.fastly.com and rt.fastly.com. Reach for this whenever a task needs numbers about how a Fastly service is performing or how much it is being used, whether via CLI, curl, or a direct HTTP integration — even if the user just says 'stats', 'metrics', 'analytics', 'traffic', or 'usage' without naming an endpoint."
---
